...
function createState(syncedManual) {
  return {
    ...
    personaSrc: { name: "検証人格" }, syncedManual, supports: [], enhancements: [
      ...
      { id: "prisoner", name: "囚人人格専用", category: "prisoner", effect: "HPを7上昇" },
...
test("同期化編集時は通常人格用だけを除外し、囚人人格用・同期化人格用・特殊強化を有効にする", () => {
  const generator = loadGenerator();
  const state = createState(true);
  assert.deepEqual(generator.getActiveEnhancements(state).map((entry) => entry.name), ["囚人人格専用", "同期化人格専用", "特殊強化共通"]);
  ...
  assert.match(generator.buildPalette(state), /囚人人格専用/);
  assert.match(generator.buildMemo(state), /囚人人格専用/);
  ...
  assert.equal(json.data.status.find((entry) => entry.label === "HP")?.value, 112);
...
test("未同期編集時は未同期専用強化を従来どおり有効にする", () => {
  ...
  assert.equal(generator.getActiveEnhancements(state).length, 4);
  assert.match(generator.buildPalette(state), /通常人格専用/);
  assert.match(generator.buildPalette(state), /囚人人格専用/);
  assert.equal(generator.buildCcfoliaJSON(state).data.status.find((entry) => entry.label === "HP")?.value, 122);
});
