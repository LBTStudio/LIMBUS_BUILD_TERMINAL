...
## 2026-09-21: 別冊を除く3冊・5カテゴリの監査

```bash
python3 tools/provenance/audit_three_books.py --write-report
python3 -m unittest discover -s tests -p test_pdf_detector.py -v
```

`three-book-audit.json` は3冊799頁からDBと独立に候補を作り、全項目を比較します。
基本E.G.O100件は1頁2件の配置・罫線で覚醒／侵蝕を分け、同化の子スキルと影響の各Rを保持。
SAN費用は基本42頁の危険度別表を引用し、固有・本文の原文行を未割当のまま成功にしません。
明示的なダイスを物理行の連結より先に検出し、字下げ追記を同じセルへ所属させます。
人格のHP/SAN別セル、広域見出し、複数行の長い名称にも対応します。

現在の対象は通常145・特異54・E.G.O115・サポート346・精神48・アイテム35。
E.G.O／ショップ3カテゴリの現在の比較は差分0ですが、人格には276件の差分候補が残ります。
`unresolved` 配列は未対応の項目マッピング、`differences` は未解決の比較差分です。
前者が0でも後者が残れば `complete:false`・終了コード1であり、完了ではありません。
自我の欠片・硝子の破片は在庫仕様による明示除外です。

`three-book-repairs.json` に修正前後・理由・原典頁・利用可能な項目別原文IDを保持します。
差分候補を自動で一括適用しないでください。語彙、数値、所属を変えない組版正規化と、
内容の欠落や誤所属は分けて評価します。用語集補足や派生番号の検証は継続中です。
PDFの折り返しとDB連結の差だけではDB修正を要求しません。
...
| 1. データベース | 組版差を記録し、参照側の欠落・一文分断を区別する | `audit-db-paragraphs.mjs` |
