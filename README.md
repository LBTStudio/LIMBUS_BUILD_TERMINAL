...
既存データとの互換性を維持するため、ブラウザー内の保存キーは `lbt_v46_state` のままです。保存データ形式は `schemaVersion: 5` です。旧保存データを開いた場合も、現在装備中の人格に必要な自己管理状態を再確認します。DB固有値は既定ステータスへ重複追加せず、固有側の初期値・上限を維持します。

## 3冊監査・取込改良（開発中）

別冊を除く基本・サプリ・パックの799頁を対象とするGoal A1〜A5を
[厳密な完了条件と進捗](docs/data-provenance-goal.md) に記載しています。**全体は未完了です。**
`python3 tools/provenance/audit_three_books.py --write-report` で全項目比較と未解決一覧を再生成します。
現在は人格の差分候補276件が残り、終了コード1を返します。単語の存在確認だけで合格にはしません。

- 姫の固有の後半、ツヴァイヘンダーの状態名、E.G.Oの影響・同化・固有・ダイス所属を出典付きで修復。
- 硝子窓の `/persona/view/` と `/view/` の人格部分を読み込み、名称を省略せず本文からキーワードを補完。
  PC名・PL名は人格追加で変更しません。条件付き固有の条件文も保持します。
- E.G.O画面の「E.G.Oを新規作成」から空の自作定義を追加・編集でき、所持ライブラリへ保存します。
  装備中の自作定義の削除を防ぎ、未装備の削除には確認を要求します。
...
# 段階1: PDFとの組版差と、参照側の本文欠落・一文分断を区別する
...
本文欠落・一文分断は0件を維持します。PDFの折り返しとDB連結の差は `layout-only` として報告し、DBへ改行追加を強制しません。三冊の全項目監査の未解決は別に管理します。
