...
function normalizeRankDraft(value) {
  const trimmed = String(value ?? "").trim();
  const shorthand = trimmed.match(/^(\d+)(?:-(\d+))?$/);
  return shorthand ? `スキル${shorthand[1]}${shorthand[2] ? `-${shorthand[2]}` : ""}` : trimmed;
}
...
const FocusedSkillCard = ({ skill, onPatch, onRemove, onAddDice, onPatchDice, onRemoveDice }) => {
  ...
  const [rankDraft, setRankDraft] = React.useState(skill.rank || "");
  React.useEffect(() => {
    setRankDraft(skill.rank || "");
  }, [skill.id, skill.rank]);
  const commitRank = () => {
    const normalized = normalizeRankDraft(rankDraft);
    setRankDraft(normalized);
    if (normalized !== (skill.rank || "")) onPatch({ rank: normalized });
  };
  return h("div", { className: "deck-card deck-focus", "data-sin": skill.sin || "" },
    h("div", { className: "deck-card-head" },
      h("label", { className: "deck-order-field" },
        ...
        h("input", { list: "skill-rank-list", className: "deck-rank deck-order-input", value: rankDraft, onChange: (e) => setRankDraft(e.target.value), onBlur: commitRank, onKeyDown: (e) => { if (e.key === "Enter") e.currentTarget.blur(); }, placeholder: "例: 1-2 / スキル4-2", title: "オーダー。1-2またはスキル1-2の形式で入力し、Enterまたはフォーカス移動で確定します。同じ派生番号を複数のスキルに指定できます。", style: { background: "var(--gold-tint)", color: "var(--gold)", border: "1px solid var(--gold-line)", cursor: "text", fontSize: "var(--fs-12)", padding: "4px 10px", fontFamily: "var(--f-display)", fontWeight: 600, borderRadius: "var(--r-sm)", minWidth: 180, letterSpacing: "0.06em" } })
      ),
