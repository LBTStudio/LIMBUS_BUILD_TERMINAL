...
const RosterSection = ({ state, dispatch }) => {
  ...
  const renderDetail = (item) => {
    ...
    return h("div", { className: "roster-detail", role: "region", "aria-label": `${item.name}の詳細` },
      ...
      item.type === "personas" && h("label", { className: `sync-max-detail-toggle${item.entry.syncMax ? " is-on" : ""}${canModifySyncMax ? "" : " is-readonly"}`, onClick: (event) => event.stopPropagation(), "aria-disabled": !canModifySyncMax, title: canModifySyncMax ? "同期MAXを設定・解除" : "同期MAXは装備中に「同期化して手動編集」後、または未保存のカスタム人格で変更できます" }, h("input", { type: "checkbox", checked: item.entry.syncMax === true, disabled: !canModifySyncMax, onChange: (event) => canModifySyncMax && dispatch({ type: "PATCH_ROSTER", uid: item.entry.uid, patch: { syncMax: event.target.checked } }) }), h("span", { className: "sync-max-detail-label" }, "同期MAXとして設定"), item.entry.syncMax && h("span", { className: "sync-max-detail-badge" }, "[MAX]"), h("span", { className: "sync-max-detail-note" }, canModifySyncMax ? "同期ランクとは別・名称と共有に反映" : "閲覧中・名称と共有に反映")),
