...
const RosterSection = ({ state, dispatch }) => {
  ...
  const renderDetail = (item) => {
    ...
    return h("div", { className: "roster-detail", role: "region", "aria-label": `${item.name}の詳細` },
      ...
      item.type === "personas" && h("label", { className: `sync-max-detail-toggle${item.entry.syncMax ? " is-on" : ""}`, onClick: (event) => event.stopPropagation(), title: "同期MAXは同期ランクとは別に保存され、名称と共有へ[MAX]表記として反映されます" }, h("input", { type: "checkbox", checked: item.entry.syncMax === true, onChange: (event) => dispatch({ type: "PATCH_ROSTER", uid: item.entry.uid, patch: { syncMax: event.target.checked } }) }), h("span", { className: "sync-max-detail-label" }, "同期MAXとして設定"), item.entry.syncMax && h("span", { className: "sync-max-detail-badge" }, "[MAX]"), h("span", { className: "sync-max-detail-note" }, "同期ランクとは別・名称と共有に反映")),
  ...
  return h("div", { className: "stack-3" },
    h(Card, null,
      h("div", { className: "card-header", style: { alignItems: "center", flexWrap: "wrap", gap: "var(--s-2)" } }, h("span", { className: "t-label" }, "OWNED LIBRARY / 所持ライブラリ"), h("div", { className: "segmented", style: { marginLeft: "auto" } },
        h("button", { className: libraryTab === "personas" ? "is-active" : "", onClick: () => setLibraryTab("personas") }, "人格"),
        h("button", { className: libraryTab === "egos" ? "is-active" : "", onClick: () => setLibraryTab("egos") }, "E.G.O")
      )),
      h("div", { className: "card-body stack-3" },
        ...
        h("div", { className: "roster-controls" },
          ...
          libraryTab === "personas" && h("div", { className: "segmented", "aria-label": "同期・MAXフィルタ" }, [["all", "同期・MAXすべて"], ["0", "同期0"], ["00", "同期00"], ["000", "同期000"], ["max", "同期MAX"]].map(([key, text]) => h("button", { key, className: syncRankFilter === key ? "is-active" : "", onClick: () => setSyncRankFilter(key) }, text))),
