...
const RosterSection = ({ state, dispatch }) => {
  ...
  const renderDetail = (item) => {
    ...
    return h("div", { className: "roster-detail", role: "region", "aria-label": `${item.name}の詳細` },
      ...
      item.type === "personas" && h("label", { style: { display: "flex", alignItems: "center", gap: 8, marginTop: 12, color: "var(--tx-1)", cursor: "pointer" }, onClick: (event) => event.stopPropagation(), title: "同期MAXは同期ランクとは別に保存され、名称へ[MAX]表記として反映されます" }, h("input", { type: "checkbox", checked: item.entry.syncMax === true, onChange: (event) => dispatch({ type: "PATCH_ROSTER", uid: item.entry.uid, patch: { syncMax: event.target.checked } }) }), h("span", null, "同期MAXとして設定"), item.entry.syncMax && h("span", { className: "badge", style: { color: "var(--gold)", borderColor: "var(--gold-line)" } }, "[MAX]")),
